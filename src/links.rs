...
pub(crate) fn get_replies_by_flow(replies: Vec<&Reply>) -> HashMap<Flow, Vec<&Reply>> {
    replies.iter().fold(HashMap::new(), |mut acc, &r| {
...
mod tests {
    ...
    fn test_get_replies_by_flow() {
        ...
        let result = get_replies_by_flow(replies.iter().collect());
    ...
    fn test_get_pairs_by_flow() {
        ...
        let result = get_pairs_by_flow(replies.iter().collect());
